...
public class WindowService : IWindowService
{
	...
	private readonly Dictionary<ObjectId, IntPtr> _windowHandleMap = new();
    ...
    public Task<IEnumerable<WindowInfo>> GetCurrentWindowsAsync()
    {
        try
        {
            ...
            return Task.FromResult(windows.AsEnumerable());
        }
        catch (Exception ex)
        {
    ...
    public Task<bool> MoveWindowAsync(ObjectId windowId, Point position)
    {
        if (!TryParseWindowId(windowId, out var handle)) return Task.FromResult(false);
    ...
    public Task<bool> ResizeWindowAsync(ObjectId windowId, Size size)
    {
        if (!TryParseWindowId(windowId, out var handle)) return Task.FromResult(false);
    ...
    public Task<bool> SetWindowStateAsync(ObjectId windowId, WindowState state)
    {
        if (!TryParseWindowId(windowId, out var handle)) return Task.FromResult(false);
        bool ok = false;
    ...
    public Task<bool> ShowWindowAsync(ObjectId windowId)
    {
        if (!TryParseWindowId(windowId, out var handle)) return Task.FromResult(false);
    ...
    public Task<bool> HideWindowAsync(ObjectId windowId)
    {
        if (!TryParseWindowId(windowId, out var handle)) return Task.FromResult(false);
    ...
    public Task<bool> BringWindowToFrontAsync(ObjectId windowId)
    {
        if (!TryParseWindowId(windowId, out var handle)) return Task.FromResult(false);
    ...
    public async Task<IEnumerable<MonitorInfo>> GetMonitorConfigurationAsync()
    {
        try
        {
            _logger.LogDebug("Getting monitor configuration");

            // Clear monitor cache to pick up any display changes
            _monitorHandleToIndex.Clear();
            _nextMonitorIndex = 0;
    ...
    private bool TryParseWindowId(ObjectId windowId, out IntPtr handle)
    {
        return _windowHandleMap.TryGetValue(windowId, out handle);
    }

    private async Task<WindowInfo> BuildWindowInfoAsync(IntPtr handle, string? windowIdOverride = null)
    {
        ...
        var windowInfo = new WindowInfo
        {
            ...
            ZOrder = 0,
            IsVisible = true,
            SavedDpi = await GetCurrentMonitorDpiAsync(monitorIndex), // Capture current DPI
    ...
    public Task<bool> SendWindowToBackAsync(ObjectId windowId)
    {
        if (!TryParseWindowId(windowId, out var handle)) return Task.FromResult(false);
    ...
    public Task<bool> MoveWindowToMonitorAsync(ObjectId windowId, int monitorIndex)
    {
        if (!TryParseWindowId(windowId, out var handle)) return Task.FromResult(false);
    ...
    public async Task<WindowInfo?> GetWindowDetailsAsync(ObjectId windowId)
    {
        ...
        try
        {
            ...
            var windowInfo = await BuildWindowInfoAsync(handle, windowId.ToString());
    ...
    public Task<bool> IsWindowValidAsync(ObjectId windowId)
    {
        if (!TryParseWindowId(windowId, out var handle)) return Task.FromResult(false);
